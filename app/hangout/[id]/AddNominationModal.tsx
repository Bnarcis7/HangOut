...
export default function AddNominationModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (hangoutType === 'HOME_TAKEOUT') {
        nominationData.metadata = {
          cuisine,
          deliveryPlatform,
        }
      } else if (hangoutType === 'HOME_DRINKS') {
        nominationData.metadata = {
          drinkType,
          ingredients,
        }
      } else if (hangoutType === 'HOME_GAMES' || hangoutType === 'GAME_NIGHT') {
        nominationData.metadata = {
          playerCount,
          duration,
        }
      } else if (hangoutType === 'MOVIE_NIGHT') {
        nominationData.metadata = {
          genre,
          runtime,
          posterUrl,
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4">
          ...
          {/* Movie fields - Show search first for MOVIE_NIGHT */}
          {hangoutType === 'MOVIE_NIGHT' && (
            <div>
          ...
          <div>
            <label htmlFor="title" className="block text-[12px] sm:text-[13px] font-medium text-gray-700 mb-2">
              {hangoutType === 'RESTAURANT' ? 'Restaurant Name' :
               hangoutType === 'HOME_TAKEOUT' ? 'Restaurant Name' :
               hangoutType === 'HOME_DRINKS' ? 'Drink Name' :
               hangoutType === 'HOME_GAMES' || hangoutType === 'GAME_NIGHT' ? 'Game Name' :
               hangoutType === 'MOVIE_NIGHT' ? 'Movie Title' :
               'Title'} *
            </label>
            <input
              type="text"
              ...
              placeholder={
                hangoutType === 'RESTAURANT' ? 'e.g., The Italian Place' :
                hangoutType === 'HOME_TAKEOUT' ? 'e.g., Pizza Palace' :
                hangoutType === 'HOME_DRINKS' ? 'e.g., Mojito' :
                hangoutType === 'HOME_GAMES' || hangoutType === 'GAME_NIGHT' ? 'e.g., Catan' :
                hangoutType === 'MOVIE_NIGHT' ? 'e.g., Inception' :
                'Enter a title'
              }
            />
          </div>
          ...
          {/* Games fields */}
          {(hangoutType === 'HOME_GAMES' || hangoutType === 'GAME_NIGHT') && (
            <>
              <div>
          ...
          {/* Movie fields - Additional details */}
          {hangoutType === 'MOVIE_NIGHT' && (
            <>
              <div>
